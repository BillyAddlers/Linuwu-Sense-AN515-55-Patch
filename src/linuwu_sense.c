 ...
 *    June 2025
 *    Project: DAMX-Sense
 *    Purpose: Enhanced logging, better profile handling, and support for Div
 * Acer Manager Max tooling
 */
...
/*
 * Support for kernel 7.2
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(7, 2, 0)
#include <linux/string.h>
#endif

MODULE_AUTHOR("Carlos Corbacho, E.M. Smith, 0x7375646F (Linuwu-Sense), "
              "modified by Div Sharp (DAMX)");
...
static void __init set_quirks(void) {
  ...
  if (enable_all) {
    ...
    interface->capability |= ACER_CAP_PLATFORM_PROFILE |
                             ACER_CAP_FAN_SPEED_READ | ACER_CAP_PREDATOR_SENSE |
                             ACER_CAP_NITRO_SENSE | ACER_CAP_NITRO_SENSE_V4 |
                             ACER_CAP_TURBO_OC | ACER_CAP_TURBO_LED |
                             ACER_CAP_TURBO_FAN;
  }
}
...
/*
 * AN515-58 RGB Fix by @fabiannabil1
 * Somehow it works
 */
...
static const struct dmi_system_id acer_quirks[] __initconst = {
    ...
    /*
     * Adding support for Nitro AN515-55
     */
    {
        .callback = dmi_matched,
...
static acpi_status WMI_apgeaction_execute_u64(u32 method_id, u64 in, u64 *out) {
...
static int acer_is_on_ac_power(void) {
  ...
  status =
      WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_SYS_INFO_METHODID,
...
static int
WMID_gaming_set_misc_setting(enum acer_wmi_gaming_misc_setting setting,
                             u8 value) {
  acpi_status status;
  u64 input = 0;
  u64 result;
...
static int
WMID_gaming_get_misc_setting(enum acer_wmi_gaming_misc_setting setting,
                             u8 *value) {
  u64 input = 0;
  u64 result;
  int ret;
...
acer_predator_v4_platform_profile_set(struct device *dev,
  ...
  on_AC = acer_is_on_ac_power();
  if (on_AC < 0)
    return on_AC;

  /* Check power source */
  /* Blocking these modes since in official version this is not supported when
   * its not plugged in AC! */
  if (!on_AC && (profile == PLATFORM_PROFILE_PERFORMANCE ||
                 profile == PLATFORM_PROFILE_BALANCED_PERFORMANCE ||
                 profile == PLATFORM_PROFILE_QUIET)) {
    return -EOPNOTSUPP;
  }
  ...
  err =
      WMID_gaming_set_misc_setting(ACER_WMID_MISC_SETTING_PLATFORM_PROFILE, tp);
  if (err)
    return err;
...
static int acer_predator_v4_platform_profile_probe(void *drvdata,
  ...
  // issues while retying to set the profile as it already found balanced and
  // did'nt retry
...
static int acer_platform_profile_setup(struct platform_device *device) {
  ...
  if (quirks->predator_v4 || quirks->nitro_sense || quirks->nitro_v4 ||
    ...
    for (retry = 0; retry < max_retries; retry++) {
      ...
      pr_warn(
          "Failed to register platform profile (attempt %d/%d), error: %ld\n",
...
static ssize_t predator_usb_charging_store(struct device *dev,
  ...
  status =
      WMI_apgeaction_execute_u64(ACER_WMID_SET_FUNCTION,
                                 val == 0    ? 663300
                                 : val == 10 ? 659204
                                 : val == 20 ? 1314564
                                 : val == 30 ? 1969924
...
static acpi_status battery_health_query(int mode, int *enabled) {
  ...
  struct get_battery_health_control_status_input params = {
      .uBatteryNo = 0x1, .uFunctionQuery = 0x1, .uReserved = {0x0, 0x0}};
  ...
  status = wmi_evaluate_method(
      WMID_GUID5, 0, ACER_WMID_GET_BATTERY_HEALTH_CONTROL_STATUS_METHODID,
      &input, &output);
  if (ACPI_FAILURE(status))
    return status;
  ...
  ret =
      *((struct get_battery_health_control_status_output *)obj->buffer.pointer);
...
static acpi_status battery_health_set(u8 function, u8 function_status) {
  ...
  struct acpi_buffer input = {sizeof(struct set_battery_health_control_input),
                              &params};
...
static acpi_status acer_set_fan_speed(int t_cpu_fan_speed,
                                      int t_gpu_fan_speed) {
  ...
  if (t_cpu_fan_speed == 100 && t_gpu_fan_speed == 100) {
    ...
    if (t_cpu_fan_speed == 0) {
      ...
      status = WMI_gaming_execute_u64(
          ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x10001, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting fan speed status: %s\n",
               acpi_format_exception(status));
        return AE_ERROR;
      ...
      status = WMI_gaming_execute_u64(
          ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0xC00008, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting fan speed status: %s\n",
               acpi_format_exception(status));
        return AE_ERROR;
      ...
      status = WMI_gaming_execute_u64(
          ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x400008, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting fan speed status: %s\n",
               acpi_format_exception(status));
        return AE_ERROR;
      ...
      status = WMI_gaming_execute_u64(
          ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0x30001, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting fan speed status: %s\n",
               acpi_format_exception(status));
        return AE_ERROR;
      ...
      status = WMI_gaming_execute_u64(
          ACER_WMID_SET_GAMING_FAN_BEHAVIOR_METHODID, 0xC30009, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting fan speed status: %s\n",
               acpi_format_exception(status));
        return AE_ERROR;
...
static ssize_t predator_fan_speed_store(struct device *dev,
  ...
  if (!token || kstrtoint(token, 10, &t_cpu_fan_speed) || t_cpu_fan_speed < 0 ||
      t_cpu_fan_speed > 100) {
  ...
  token = strsep(&input_ptr, ",");
  if (!token || kstrtoint(token, 10, &t_gpu_fan_speed) || t_gpu_fan_speed < 0 ||
      t_gpu_fan_speed > 100) {
...
static acpi_status acer_predator_state_restore(int value) {
  ...
  acpi_status status =
      acer_set_fan_speed(value == 0 ? current_states.battery_state.cpu_fan_speed
                                    : current_states.ac_state.cpu_fan_speed,
                         value == 0 ? current_states.battery_state.gpu_fan_speed
...
static int acer_predator_state_load(void) {
  ...
  if (!IS_ERR(file)) {
    ...
    len = kernel_read(file, (char *)&current_states, sizeof(current_states),
                      &file->f_pos);
...
static ssize_t predator_lcd_override_show(struct device *dev,
  ...
  acpi_status status;
  u64 result;
  status = WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_PROFILE_METHODID, 0x00,
                                  &result);
  if (ACPI_FAILURE(status)) {
...
static ssize_t predator_lcd_override_store(struct device *dev,
  ...
  status = WMI_gaming_execute_u64(ACER_WMID_SET_GAMING_PROFILE_METHODID,
                                  val == 1 ? 0x1000000000010 : 0x10, &result);
  if (ACPI_FAILURE(status)) {
...
static ssize_t predator_boot_animation_sound_show(struct device *dev,
  ...
  acpi_status status;
  u64 result;
  status = WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_MISC_SETTING_METHODID,
                                  0x6, &result);
  if (ACPI_FAILURE(status)) {
...
static struct device_attribute fan_speed =
    __ATTR(fan_speed, 0644, predator_fan_speed_show, predator_fan_speed_store);
...
static struct attribute *predator_sense_attrs[] = {
    &dev_attr_version.attr,  &lcd_override.attr,         &fan_speed.attr,
...
static acpi_status set_kb_static_mode(int brightness) {
  u8 gmInput[16] = {0};
  struct acpi_buffer input = {(acpi_size)sizeof(gmInput), (void *)gmInput};
  ...
  return wmi_evaluate_method(
      WMID_GUID4, 0, ACER_WMID_SET_GAMING_KB_BACKLIGHT_METHODID, &input, NULL);
}
...
static ssize_t four_zoned_rgb_kb_show(struct device *dev,
  ...
  return sprintf(buf, "%d,%d,%d,%d,%d,%d,%d\n", output.gmOutput[0],
                 ...
                 output.gmOutput[5], output.gmOutput[6], output.gmOutput[7]);
}
...
static acpi_status get_per_zone_color(struct per_zone_color *output) {
  ...
  for (int i = 0; i < 4; i++) {
    status = WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_RGB_KB_METHODID,
                                    zone_ids[i], zones[i]);
    if (ACPI_FAILURE(status)) {
...
static acpi_status set_per_zone_color(struct per_zone_color *input) {
  ...
  if (quirks->an515_58_rgb_fix) {
    status =
        WMI_gaming_execute_u64(ACER_WMID_GET_GAMING_SYS_INFO_METHODID, 0, NULL);
    if (ACPI_FAILURE(status)) {
      pr_err("Error polling gaming system info before RGB update: %s\n",
             acpi_format_exception(status));
      return status;
    }
    ...
    status = WMI_gaming_execute_u64(
        ...
        ACER_GAMING_KBL_SET_ON | ACER_GAMING_KBL_SET_ALL_ZONES, NULL);
    if (ACPI_FAILURE(status)) {
      pr_err("Error enabling AN515-58 keyboard RGB zones: %s\n",
             acpi_format_exception(status));
      return status;
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      struct acpi_buffer acpi_input;
      u64 color = *zones[i];

      params.zone = zone_ids[i];
      params.red = (((color >> 16) & 0xff) * input->brightness) / 100;
      params.green = (((color >> 8) & 0xff) * input->brightness) / 100;
      params.blue = ((color & 0xff) * input->brightness) / 100;

      acpi_input.length = sizeof(params);
      acpi_input.pointer = &params;

      status = wmi_evaluate_method(WMID_GUID4, 0,
                                   ACER_WMID_SET_GAMING_RGB_KB_METHODID,
                                   &acpi_input, NULL);
      if (ACPI_FAILURE(status)) {
        pr_err("Error setting AN515-58 RGB color (zone %d): %s\n", i + 1,
               acpi_format_exception(status));
        return status;
      }
    }

    status = set_kb_static_mode(input->brightness);
    if (ACPI_FAILURE(status)) {
      pr_err("Error enabling AN515-58 static RGB mode: %s\n",
             acpi_format_exception(status));
      return status;
    }
...
static int four_zone_kb_state_save(void) {
  ...
  len = kernel_write(file, (char *)&current_kb_state, sizeof(current_kb_state),
                     &file->f_pos);
  if (len < 0) {
...
static int four_zone_kb_state_load(void) {
  ...
  if (!IS_ERR(file)) {
    ...
    len = kernel_read(file, (char *)&current_kb_state, sizeof(current_kb_state),
                      &file->f_pos);
    filp_close(file, NULL);
  ...
  if (current_kb_state.per_zone) {
    ...
    status = set_kb_status(current_kb_state.mode, current_kb_state.speed,
                           ...
                           current_kb_state.green, current_kb_state.blue);
    if (ACPI_FAILURE(status)) {
      pr_err("Error setting KB status.\n");
      return -1;
    }
...
static int acer_wmi_hwmon_init(void) {
  ...
  hwmon = devm_hwmon_device_register_with_info(dev, "acer", &supported_sensors,
                                               &acer_wmi_hwmon_chip_info, NULL);
...
static int __init acer_wmi_init(void) {
  ...
  if (enable_all || predator_v4 || nitro_v4) {
    pr_info(
        "Input parameters - enable_all: %d, predator_v4: %d, nitro_v4: %d\n",
